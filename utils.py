...
def sub_to_full(M_sub, dead_rows=None, dead_cols=None, unit_test=True):
    assert (dead_rows is not None) or (dead_cols is not None), f"Should receive dead_rows, dead_cols or both"
    dev = M_sub.device
...
def save_curvature_to_dir(curvature, model, shot_i, name, save_curvature, path='./curvatures_mini/'):
