...
from transformers import DefaultDataCollator, LlamaForCausalLM, OPTForCausalLM
...
from lora import absorb_lora, tune_lora, undo_lora
from sparsify import sparsify_model
from utils import log_model, log_usage, save_curvature_to_dir
...
def get_model(model_str):
    ...
    if "opt" in model_str:
        model = OPTForCausalLM.from_pretrained(
            model_str, torch_dtype="auto", use_auth_token=TOKEN, device_map="auto"
        )
...
def main(
    ...
    if len(structures) > 0:
        ...
        for shot_i in range(1, shots + 1):
            ...
            if newdata:
                assert (
                    len(trainenc) == nsamples * shots
                ), f"Train encodings seem to have wrong size {len(trainenc)}. Expected (nsamples*shots)={nsamples * shots}."
                start_i = shot_i * nsamples
                stop_i = shot_i * nsamples + nsamples
            else:
                assert (
                    len(trainenc) == nsamples
                ), f"Train encodings seem to have wrong size {len(trainenc)}. Expected nsamples={nsamples}."
                start_i = 0
                stop_i = nsamples
            ...
            if curvature_type == "identity":
                ...
                curvature = Activations(
                    model, train_dataloader, nsamples=nsamples, curvature_dev=curvature_dev
                )
                ...
                if reuse and (shot_i > (skip + 1)):
                    ...
                    curvature = KFAC(
                        ...
                        use_iad=use_iad,
                        nsamples=nsamples,
                        kpca_iter=reuse_kpca_iter,
                        max_outgrad=max_outgrad,
                        dev="cuda",
                        writer=writer,
                        shot_i=shot_i,
                        diagonal=diagonal,
            ...
            if diagonal:
                for mod_i, mod in enumerate(curvature.curvature.keys()):
                    ...
                    for i in range(krank):
                        ...
                        kron_diag = sum(
                            [torch.kron(G[j].diag(), A[j].diag()) for j in range(i + 1)]
                        ).view(-1)
            ...
            if schedule == "linear":
                ...
                raise NotImplementedError(
                    f"Unknown schedule for pruning shots: {schedule}. Try using linear instead."
                )
...
parser.add_argument(
    "--log_root", type=str, required=False, default=os.environ.get("LOG_ROOT", None)
)
...
if args.log_root is None:
    raise ValueError(
        "Either --log_root should be passed or LOG_ROOT should be set in the environment"
    )
