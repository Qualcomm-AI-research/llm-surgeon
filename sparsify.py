...
from threshold import structured_threshold, semistructured_threshold, unstructured_threshold
...
@torch.no_grad()
def sparsify_model(model, sparsity, structures, curvature, obd, update_scale, max_correlate, damp_g, damp_a, use_diagonal, rank1cost, eigenfix, addupdate, zerofix, strictmax):
    ...
    for layer_i, layer in enumerate(layers):
        ...
        for _, mod in mods.items():
            ...
            full_W_shape = (mod.weight.shape[0], mod.weight.shape[1] + 1) if mod.bias is not None else (mod.weight.shape[0], mod.weight.shape[1])
            ...
            if ('row' in structures) and ('column' in structures):
                row_losses = pruner.local_costs(W, G, A, diagonal, 'row', curvature, obd, use_diagonal, rank1cost, damp_g, damp_a, Ginv=Ginv, Ainv=Ainv)
                col_losses = pruner.local_costs(W, G, A, diagonal, 'column', curvature, obd, use_diagonal, rank1cost, damp_g, damp_a, Ginv=Ginv, Ainv=Ainv)

                losses = (row_losses, col_losses)
            elif '2:4' in structures:
                losses = pruner.local_costs(W, G, A, diagonal, 'element', curvature, obd, use_diagonal, rank1cost, damp_g, damp_a, Ginv=Ginv, Ainv=Ainv)
            ...
            structured_losses[mod] = {'losses': losses,
                                      'dead': dead,
    ...
    for mod_i, mod in enumerate(all_mods):
        ...
        pruner.weight_update(structures, curvature, obd, global_threshold, update_scale, max_correlate, losses, Ginv, Ainv, use_diagonal, addupdate, damp_g, damp_a, zerofix, strictmax)
        pruner.free()
