...
def semistructured_threshold(all_losses, target_sparsity, mn=(2, 4), device='cpu'):
    ...
    for _, structured_losses in all_losses.items():
        ...
        assert (losses_no_bias.shape[1] % n) == 0, f"Expected even number of columns without bias to be divisible by N ({n}). Got {losses.shape}), full_W_shape={full_W_shape}, has_bias={has_bias}."
    ...
    target_i = min(int(target_block_sparsity * total_blocks), len(sorted_losses) - 1) # can this edgecase occur if rounding is unlucky? otherwise, remove min() for cleanness
...
def structured_threshold(all_losses, target_sparsity, device='cpu'):
    ...
    for _, structured_losses in all_losses.items():
        ...
        row_losses = sub_to_full(row_losses.view(-1, 1), dead_rows, torch.zeros(1, dtype=torch.bool, device=device)).flatten()
        col_losses = sub_to_full(col_losses.view(1, -1), torch.zeros(1, dtype=torch.bool, device=device), dead_cols).flatten()
