...
def get_wikitext2(model, nsamples=128, seed=100, seqlen=2048, load=True):
    ...
    print('\t--TEST: ', sum([len(x) for x in testdata]), len(testdata), [len(x) for x in testdata[:20]])
...
def get_wikipedia(model, lang, nsamples=128, seed=100, seqlen=2048, split=0.1, subsets=(3000, 400), olm=True, load=True):
    print(f'Loading [{lang}]... (nsamples={nsamples}, seed={seed}, seqlen={seqlen}, split={split}, subsets={subsets})')
    set_seed(seed)
...
def get_code_search_net(model, lang, nsamples=128, seed=100, seqlen=2048, split=0.1, subsets=(6000, 800), load=True):
    set_seed(seed)
